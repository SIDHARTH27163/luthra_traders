...
<div class="p-2  rounded-lg  border-2 border-slate-900 border-dashed h-auto">
  <div class="grid  grid-cols-2 gap-4 mb-4">
   ...
   <div class="w-full p-4 ">
      ...
      <p class="toggleColour text-gray-900 text-2xl  font-bold underline py-2">Manage Shop About</p>
      <div class="h-full  w-full  flex flex-col  items-center justify-center ">
       ...
       {{-- form for add location starts --}}
          <form action="{{url('edit_abouts/'.$udata->id)}}" enctype="multipart/form-data" method="post" class="lg:w-3/4 md:w-3/4 sm:w-3/4 w-full bg-white p-4  rounded-lg shadow-lg  ">
              @csrf
              @method('post')
              <div class="mb-4">
                <label for="name" class="block mb-2 text-lg font-medium text-gray-900 dark:text-white">Edit Heading</label>
                <textarea id="message" name="heading" rows="2" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500  " >{{$udata->heading}}</textarea>
              </div>
              <div class="mb-4">
                <label for="name" class="block mb-2 text-lg font-medium text-gray-900 dark:text-white">Edit Description</label>
                <textarea id="message" name="description" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500  " >{{$udata->description}}</textarea>
              </div>
              <div class="mb-2">
                <label for="name" class="block mb-2 text-lg font-semibold text-gray-900 dark:text-white">Edit Image</label>
                ...
                <p>Current Image: <a target="_black" href="{{ asset('traders/'.$udata->image) }}">{{ $udata->image }}</a></p>
              </div>
