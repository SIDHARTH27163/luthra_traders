...
<div class=" w-full p-1 flex  flex-row justify-between">
    ...
    <a href="/get_pg_data" class="toggleColour text-rose-600 text-2xl  font-bold underline py-2">Back</a>
   
</div>
  
<div class="w-full p-2 flex flex-col items-center justify-center">
  
    <form action="/import_users" method="post" enctype="multipart/form-data" class="lg:w-3/4 md:w-3/4 sm:w-3/4 w-full bg-white p-4  rounded-lg shadow-lg  ">
       @csrf
       @method('post')
       ...
       <div class="mb-4">
         ...
         <input type="file" name="file" id="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"  multiple>
         @if($errors->has('file'))
       <p class="text-sm italic text-red-500 text-start font-semibold">{{ $errors->first('file') }}</p>
 @endif
